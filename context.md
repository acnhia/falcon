...
- Repository layout is `backend/{java-service,edge-worker,contracts}`, `frontend/`, `infrastructure/{cloudflare,local}` and `docs/{requirements,architecture,adr,research}`, with a root `Makefile` as the single task runner. A separate multipart file-transfer demo lives alongside the onboarding POC, isolated by module inside each runtime.
- **Live deployment:** https://<your-subdomain>.workers.dev (Cloudflare Worker `aom-demo`) serves the onboarding wizard at root `/` and the file-transfer demo at `#/upload` from one deploy (see routes table below). Redeploy via `make deploy` (containerized; builds, tests, provisions R2/D1, deploys). Bindings: R2 bucket `test` (`UPLOADS`), D1 `upload-transfer-test` (`TRANSFERS`), D1 `onboarding-test` (`ONBOARDING_DB`), Worker secret `OPENAI_API_KEY` (set from root `.env`'s `OPENAPI_KEY` by `scripts/provision-openai-secret.mjs`).
- The account-onboarding **identity-capture slice** (start application → capture-link token → front/back document capture → mock validation → `READY_FOR_REVIEW`) is implemented end-to-end and tested in the **Java service** (`backend/java-service/`, JDBC/H2 persistence) — this remains the reference implementation for local/container dev and its own test suite.
- The **wizard/workflow "foundation" phase** (activities 1–4 of 21, wizard screens 1–2 (of the superseded eight-screen model; see the four-section note below); see [wizard, normalized data, and service design](docs/architecture/brokerage-onboarding/05-wizard-data-and-services.md)) is implemented and tested **twice, in parallel**, because Cloudflare Workers can't run the Spring Boot JAR:
  - **Java service** (`com.falcon.onboarding`, `backend/java-service/`): `spring-boot-starter-jdbc` + H2 (file-based locally, in-memory for tests), an idempotent `schema.sql` covering 9 of the doc's 12 tables (`onboarding_application`, `onboarding_activity`, `field_definition`, `application_field_value`, `identity_capture_session`, `identity_document`, `provider_check`, `workflow_operation`, `application_audit_event`), a `repository.jdbc` package, a `workflow` package (`ActivityProgressService`, `AgeIdentityPrecheckService`), and `WizardOrchestrator`/`DefaultWizardOrchestrator` (kept separate from `OnboardingOrchestrator` so the capture-flow contract/tests stay untouched) exposing `GetResumeState`/`SaveActivityDraft`/`ContinueActivity` through the same `TaskRegistry` pattern. 84 tests passing (`make test-java`, which runs Maven in a container - no local JDK needed). Used for local/container dev only — **not what the live deploy runs**.
  - **Cloudflare Worker** (`backend/edge-worker/src/onboarding/`, TypeScript against D1): the canonical production runtime, layered `domain/` (pure progress rules and errors), `repository/` (all D1 access), `service/` (use-cases), `assistant/` (realtime voice session and field extraction), `validation/` (header-based JPEG/PNG dimension parsing, since Workers has no image decoder), `workflow/` and `web/` (router dispatching `/api/onboarding/*`). Own D1 database `ONBOARDING_DB`/`onboarding-test`, reuses the existing R2 bucket under an `onboarding/` prefix. Concurrency (single-fire mock validation, single-use capture token) via conditional D1 `UPDATE ... WHERE x = 0` + checked `meta.changes`, no Durable Object needed. **This is what https://<your-subdomain>.workers.dev actually runs.** 49 tests passing (`make test-worker`), including the contract parity suite and `fieldExtraction.ts` (the second live AI call - see "AI and voice" below).
  - Frontend (`frontend/src/onboarding/`, shared by both backends since it just calls relative `/api/...`): `WelcomePage.tsx` (screen 1: consent, create-or-resume via a `localStorage`-held public reference), `PersonalInformationPage.tsx` (screen 2: dark two-panel layout, field-catalogue-driven form with 40+ fields across 10 grouped sections - see `docs/06`'s UI-S1-002 for the full list - presented as three Back/Next steps with a titled progress bar, save-draft on any step and Submit on the last, plus a chat-bot styled assistant panel with a text composer, voice button and mute control), `OnboardingWizard.tsx` (welcome screen, then the personal-information wizard), `RestartButton.tsx` (confirm-then-clear-session control on every wizard stage), `realtimeVoice.ts` (real OpenAI Realtime API/WebRTC voice, including live streaming transcript deltas - see "AI and voice" below). `mockAssistant.ts`'s canned-suggestion mechanism was deleted, superseded by the composer's extraction-call proposals. Chat log auto-scrolls to the newest message unless the user has scrolled up (sticky-scroll); the composer is a single bordered container with a slim icon toolbar (mic, mute, send) along its bottom edge. `App.jsx`'s bare `/` serves the wizard; file-transfer demo moved to `#/upload`. 93 tests passing (`make test-frontend`), plus `make check` for typecheck and lint. The 821-line page was split: the field catalogue is `fields/catalogue.ts` and control rendering is `fields/FieldInput.tsx`, with `fields/catalogue.test.ts` asserting the catalogue against `backend/contracts`.
  - Deferred: the remaining wizard sections (the original eight-screen split is **superseded** by the four-section model in `docs/architecture/brokerage-onboarding/06-wizard-component-user-stories.md`; screen numbering survives only in `wizardScreenFor` and historical rows), `application_address`/`application_disclosure_acknowledgement`/`application_agreement_acceptance` tables, the general stale-dependency graph (only the activity-3→4 date-of-birth edge exists), the market ticker, and the full `marketdata`/`verification`/`disclosures`/`notifications`/`decisioning` modules.
...
- Detailed source requirements live in:
  - [Overview](docs/architecture/brokerage-onboarding/00-overview.md)
  - [Frontend](docs/architecture/brokerage-onboarding/01-frontend.md)
  - [Backend](docs/architecture/brokerage-onboarding/02-backend.md)
  - [Deployment](docs/architecture/brokerage-onboarding/03-deployment.md)
  - [Detailed 21-step onboarding flow](docs/architecture/brokerage-onboarding/04-onboarding-flow.md)
  - [Wizard, normalized data, and service design](docs/architecture/brokerage-onboarding/05-wizard-data-and-services.md)
  - [Four-section wizard component user stories](docs/architecture/brokerage-onboarding/06-wizard-component-user-stories.md)
...
| 2026-08-19 | Gave the Java service its own identity: Maven artifact/name/jar `upload-demo` → `falcon-backend`, and the Spring entry point moved from `com.falcon.upload.UploadDemoApplication` to `com.falcon.FalconApplication` at the namespace root, deleting the `@SpringBootApplication(scanBasePackages = "com.falcon")` override it previously needed. | Found while verifying the code-standards review: the onboarding POC was booting from the *file-transfer demo's* package, and its description still read "Multipart upload to Cloudflare R2 - learning demo". The pre-existing `OnboardingComponentScanTest` proves the scan still reaches onboarding without the override. | Active — 84 Java tests green |
| 2026-08-19 | Added a root `Makefile` as the single task runner (`make test` / `check` / `up` / `deploy`), and reorganised documentation into `docs/{requirements,architecture,adr,research}` with normalised filenames (`README.md`, kebab-case). The eight-screen wizard model is marked **superseded** by the four-section model rather than deleted. | The review found build scripts hardcoding directory names and four competing documentation sources of truth. Defining paths once in the Makefile is what makes the restructure survivable — the path coupling is exactly what made it risky. | Active |
| 2026-08-19 | Implemented the contract and parity suites promised by the canonical-runtime row above: `backend/contracts/onboarding-field-catalogue.json` is now the single source of truth for field keys, required-ness and allowed values, enforced by three suites (Java `OnboardingContractParityTest`, worker `contractParity.test.ts`, frontend `fields/catalogue.test.ts`). The catalogue was **generated from the runtimes**, so it provably describes current behaviour rather than an aspiration. | Verified the suites actually fail on drift rather than passing vacuously: removing one value from `maritalStatus` made both backends fail, naming the offending field. A parity test that cannot fail is worse than none. Scope is deliberately narrow — labels, control types and step grouping stay in the frontend, where a difference between runtimes is harmless. | Active — closes the review's P0 drift finding |
| 2026-08-19 | Moved edge-worker test bindings from an untracked `.dev.vars` into `vitest.config.ts`. | Two reasons: the deployment descriptor now lives in `infrastructure/`, so `.dev.vars` discovery relative to it would cross boundaries; and an untracked file must never be what makes a suite pass, or a fresh clone fails for reasons nobody can see. Values are unchanged and still fake (admin/admin plus Cloudflare's published "always passes" Turnstile test pair). This resolves the fresh-clone gap flagged when `.dev.vars` was gitignored. | Active |
| 2026-08-19 | Widened `backend/edge-worker/tsconfig.json`'s `include` from a single entry file to the whole source tree, so `npm run check` typechecks tests for the first time. | Previously broken imports in test files passed the typecheck gate silently. Widening immediately caught a real latent bug: `activityStatus()` called `.status` on a possibly-undefined `find()` result, which would have failed as a `TypeError` rather than a readable assertion. The type debt this exposed was paid off rather than suppressed. | Active |
| 2026-08-19 | De-identified the reference brokerage across the whole codebase — comments, docs, and one test method name — replacing every `WellsTrade`/`wellstrade` occurrence with generic wording, then later redacting its real support phone number and an internal URL fragment from the research inventory before publishing. | The user asked to remove the institution's name; the phone number and URL fragment identify it just as directly, so leaving them would have defeated the instruction. Flagged both before acting. | Active |
| 2026-08-19 | Audited the repository before the first push of this work to the **public** GitHub repo, and pushed 10 commits to `main`. | Confirmed absent from *all* history: Cloudflare API token, R2 access/secret keys, account ID, OpenAI key, session secret and the Turnstile **secret** key; `.env` and `.dev.vars` were never committed. Values that did match are non-secrets (D1 database *names*, resource UUIDs in `wrangler.jsonc`, public OpenAI URLs, and the Turnstile **site** key, which is designed to be embedded in page HTML). Also removed the deployment's personal `*.workers.dev` subdomain from this file before it could be published. **Known, unresolved:** `BaseTask.java` carries a comment pointing at an internal `ottera-repos/...` path, pushed in commits predating this session and therefore already public; removing it from history needs a force-push over published commits, which was not done unasked. | Active |
...
Implemented identically (same paths/methods/request/response shapes) in **both** the Java backend (`OnboardingController`/`CaptureController`, local/container dev) and the Cloudflare Worker (`backend/edge-worker/src/onboarding/web/router.ts`, what the live deploy runs) unless noted otherwise — the React client just calls relative `/api/...` paths, so it works unmodified against either.
...
The detailed [21-step onboarding flow](docs/architecture/brokerage-onboarding/04-onboarding-flow.md) covers landing disclosure through capture-link issuance, mobile front/back synthetic-document capture, private storage, deterministic mock validation, review-ready state, recovery behavior, and controlled test-data reset.
...
- **Cloudflare (what's actually deployed):** one Worker (`aom-demo`) hosts the React static assets and runs the onboarding API for activities 1–4/identity-capture natively in TypeScript against D1 (`backend/edge-worker/src/onboarding/`) — not a proxy to a separately-hosted Java service. The Java backend remains the reference implementation for local/container dev; a future adapter that instead calls a separately-hosted Java service (for activities 5–21, before/if they're ported to Workers) would use a Worker only for edge/static/API routing, with R2/D1 bound only where used.
